...
def git_commit(base_dir):
    ...
    if OS == 'nt':
        ...
        result = subprocess.run(['git', 'status', '--porcelain'], stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, shell=False)
    if result.stdout:  # If there's any output, there are changes
        ...
        if OS == 'nt':
            ...
            subprocess.run(['git', 'add', '.'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=False)
            subprocess.run(['git', 'commit', '-m', f'Automated commit at {timestamp}'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=False)
...
def git_push(base_dir):
    ...
    if OS == 'nt':
        ...
        subprocess.run(['git', 'push'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=False)
...
def git_pull(base_dir):
    ...
    if OS == 'nt':
        ...
        subprocess.run(['git', 'pull'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=False)
